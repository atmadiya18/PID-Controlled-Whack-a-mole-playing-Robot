...
void loop() {
  ...
  if (measured_distance > 30 && Linefollowerarray[2] == 1 && Linefollowerarray[3] == 0 && Linefollowerarray[4] == 1) {
    ...
    /*while (Linefollowerarray[4] == 1)
    {
      digitalWrite(in1, HIGH); //Clockwise for Motor 1
      digitalWrite(in2, LOW);
      analogWrite(en1, 160);

      digitalWrite(in3, HIGH); //Anticlockwise for Motor 2
      digitalWrite(in4, LOW);
      analogWrite(en2, 160);

      digitalWrite(in5, HIGH); // clockwise for Motor3
      digitalWrite(in6, LOW);
      analogWrite(en3, 224);
...
double computePID(double inp, double kp, double ki, double kd){ //Only for PID Control
